...
**Two modes — pick based on whether the cycle had significant change:**

**Significant change** = any of: fill (entry or exit), SL/TP hit, new FVG detected, regime change. Use the FULL TABLE form.

**No significant change** = none of the above. Use the 1-LINE form.

### 1-line form (no significant change)
```
QQQ $XXX VWAP $YYY (±N.NN%) — no setup, próx scan HH:MM
```
Single line. No table. No comments.

### Full table form (significant change)
```
...
After the table, máximo 1 línea de comentario (fill, exit, regime change, new FVG). No re-narrar gates fijos.
...
## STEP 0b — EARLY EXIT (strict AND-triple — v2.9.1, 2026-06-09)
After fetching bars (STEP 3), evaluate ALL four conditions:
  (a) max(bar.t) == last_bar_UTC          (no new sealed bar)
  (b) get_all_positions == []              (no open position)
  (c) active_fvgs == []                    (no pending FVG limit orders)
  (d) (now_ET − regime_last_eval_ET) < 30 min  (regime check still fresh)

If (a) AND (b) AND (c) AND (d) ALL true → ociosos cycle:
  - Do NOT execute STEP 4–9b (skip compute and write entirely)
  - Print 1 line: "HH:MM ET — sin cambios — skip"
  - ScheduleWakeup(270s)

If only (a) is true but any of (b)/(c)/(d) false → continue cycle (need to manage positions, check FVGs, or re-eval regime).
...
## STEP 3 — MARKET DATA (token-reduced — v2.9.1, 2026-06-09)
For QQQ (universe is QQQ-only):
  If has_state AND state[symbol].bars_count > 0:
    # 5-min: limit=2 baseline (último sealed + 1 contexto)
    if (now_UTC − last_bar_UTC) > 30 min:
      get_stock_bars(symbol, "5Min", 20, "sip") — gap recovery (e.g., loop interrupted)
    else:
      get_stock_bars(symbol, "5Min", 2, "sip")
    # 1-min: limit=5 baseline (3 triplets FVG posibles)
    if any active_fvg AND abs(live_price − fvg.sl_level) ≤ $0.30:
      get_stock_bars(symbol, "1Min", 10, "iex") — verify SL invalidation on older bars
    else:
      get_stock_bars(symbol, "1Min", 5, "iex")
...
**Session high timestamp** (v2.9.1 — for FVG tracking):
  Si last_close > state.QQQ.session_high (o session_high es null):
    state.QQQ.session_high = last_close
    state.QQQ.session_high_ts_ET = current_ET (HH:MM)

**VWAP snapshot maintenance** (v2.9.1 — for FVG vwap_slope tracking):
  snap = state.QQQ.vwap_snapshot_30min
  if snap is null OR (current_ET − snap.ts_ET) ≥ 30 min:
    state.QQQ.vwap_snapshot_30min = { value: VWAP, ts_ET: current_ET }
...
## STEP 6 — MANAGE OPEN POSITIONS (conditional fetch — v2.9.1, 2026-06-09)
...
**UNPROTECTED-POSITION SAFETY NET (v2.9.2, 2026-06-10 — PRIORIDAD ABSOLUTA, antes que nada):**
Si `get_all_positions` muestra una posición QQQ abierta PERO `session_state.position.oco_a_id`
es null (o al fetchear los OCO no existen / están cancelled) → la posición se llenó durante el
gap inter-cycle y está DESPROTEGIDA. INMEDIATAMENTE:
  1. Armar OCO ahora (ejecutar STEP 8b punto 4 con el qty real de la posición + el `sl_level`
     del FVG originante guardado en active_fvgs / position).
  2. Si el placement de OCO falla 2 veces → `place_stock_order(market sell, qty_total)` ya,
     loguear "emergency close: unprotected fill", limpiar state.position.
Hacer esto ANTES de cualquier cómputo token-heavy. Este es el cierre real del gap del 06-09
(fill→OCO): el wakeup de 60s (STEP 9c) reduce el gap a ≤60s, este net lo cubre si igual ocurre.

**Conditional fetch policy (token-saving):**
Para cada posición, decidir si fetchear los OCOs:
  fetch_ocos = (abs(live_price − TP1) ≤ $0.10)
             OR (abs(live_price − TP2) ≤ $0.10)
             OR (abs(live_price − SL)  ≤ $0.10)
             OR (state.position.qty != get_all_positions[symbol].qty)  /* safety net */

If fetch_ocos == false:
  Asumir OCOs status="new", skip get_order_by_id calls, continuar al STEP 7.

If fetch_ocos == true (live cerca de algún level, o qty divergente → algo se llenó):
...
**Exit tracking (v2.9.1, 2026-06-09):** Cuando se registre exit de un FVG (cualquier exit_type), append al notes del trade row el string de tracking computado en STEP 8b:
  ```
  | tracking: bucket=<formed_at_bucket> risk=<risk_size_bucket> slope=<vwap_slope> outcome=<TP1|TP2|BE|SL|TIME>
  ```
  Usar `session_state.position.fvg_tracking` (guardado en STEP 8b) como fuente. Aplica via UPDATE trades SET notes = notes || '<tracking_string>' WHERE order_id = entry_order_id (ver feedback-trades-match-buy-sell).

**Safety net (STEP 0 también):** comparar `get_all_positions[symbol].qty` vs `state.position.qty`. Si difieren → algo se llenó entre cycles → force OCO check independientemente del live_price.
...
**GUARDRAIL — MAX 1 FVG LIMIT ACTIVO (v2.9.2, 2026-06-10 — P3):**
Solo 1 FVG operable a la vez. Si ya existe algún FVG en `active_fvgs` con status="pending"
(limit vivo) O hay una posición FVG abierta → NO colocar limits nuevos este cycle. Seguir
detectando FVGs para logging/tracking, pero `status="skipped_max1"`, sin order. Esto elimina
el escenario 06-09 donde 3 limits stacked se llenaron en 4s con un solo bar low.

Para cada FVG **nuevo** (no presente ya en `active_fvgs`), SOLO si el guardrail anterior lo permite:
  1. shares = floor(equity × 0.025 / midpoint). Skip si shares < 2.  *(v2.9.2 P2 — sizing reducido 0.05→0.025 hasta validar el fix del gap OCO; revertir a 0.05 tras 3 sesiones limpias.)*
...
## STEP 7c — SHADOW SIGNALS (research nuevos sistemas — NO OPERA, solo logging; v2.9.2, 2026-06-10)

Candidatos en validación (ver memoria `project_new_systems_research.md`). Si alguno dispara,
NO colocar orden — incluir en el `indicators` JSONB del STEP 9:
```json
"shadow_signals": [{"system":"C1_fvg_trend|C2_sweep_reclaim|C3_val_bounce",
                    "entry":X,"sl":X,"tp":X,"ts_ET":"HH:MM","context":"1 línea"}]
```
- **C1 FVG-Trend**: FVG bullish detectado AND vwap_slope_30min=up AND precio>VWAP. (Si el FVG real
  ya se operó por STEP 7b, marcar igualmente si C1 lo hubiera tomado o filtrado — eso es el dato.)
- **C2 Sweep & Reclaim**: 1-min hace nuevo session low, y la siguiente 1-min sella close > low previo
  con vol ≥1.5× avg 5 bars → entry=open siguiente, SL=sweep_low−0.05, TP=2R.
- **C3 VAL Bounce**: touch de yesterday VAL o developing VAL con barra de rechazo (close en mitad
  superior del rango) → entry=close, SL=VAL−0.5×ATR, TP=VPOC.
Outcome simulado se resuelve en post-close. Costo target: ≤200 tokens/cycle extra (solo cuando dispara).
...
Trigger: cycle maintenance encontró `get_order_by_id(limit_order_id).status = "filled"` para un FVG en active_fvgs. Sizing ya fijado en formation (`shares`); SL fijo (`fvg.sl_level`).
  ...
  4. **MANDATORY — Place OCO sell brackets (mismo patrón que STEP 8 punto 4):**
     ...
     - **(v2.9.2)** Armar OCO es la PRIMERA acción tras confirmar el fill (antes de logging/tracking).
       Si el placement de OCO falla → retry 1 vez; si vuelve a fallar → `place_stock_order(market sell, shares)`
       inmediato + loguear "emergency close: OCO arm failed". NO dejar la posición sin protección broker-side.
       **NO usar `order_class="bracket"`** para FVG: en paper Alpaca el SL leg queda `status="held"` post-fill
       y no auto-activa (bug confirmado 06-09). Usar siempre 2 OCO sell separados como aquí.
  5. **Compute FVG tracking fields** (v2.9.1 — for analysis_log.indicators in STEP 9):
     ```
     # formed_at_bucket (basado en hora ET del bar n+2)
     hh_et = formed_at_ET.hour + formed_at_ET.minute/60
     if 10.0 ≤ hh_et < 12.0:  formed_at_bucket = "10-12"
     elif 12.0 ≤ hh_et < 14.0: formed_at_bucket = "12-14"
     elif 14.0 ≤ hh_et ≤ 15.5: formed_at_bucket = "14-15:30"
     else:                     formed_at_bucket = "other"

     # vwap_slope (delta vs snapshot ~30min atrás)
     snap = state.QQQ.vwap_snapshot_30min  # {value, ts_ET}
     if snap exists AND (now_ET − snap.ts_ET) ≥ 25 min:
       delta = state.QQQ.vwap − snap.value
       if abs(delta) < 0.10: vwap_slope = "flat"
       elif delta > 0:       vwap_slope = "up"
       else:                 vwap_slope = "down"
     else: vwap_slope = "unknown"

     # time_since_session_high_min
     time_since_high_min = (now_ET − state.QQQ.session_high_ts_ET) en minutos enteros

     # risk_size_bucket
     if fvg.planned_risk < 0.30:     risk_size_bucket = "tight"
     elif fvg.planned_risk < 0.50:   risk_size_bucket = "mid"
     else:                            risk_size_bucket = "wide"
     ```
     Guardar también en `session_state.position.fvg_tracking` para que el exit (STEP 6) pueda incluirlo en el notes del trade row.
  6. INSERT trade row (Supabase directo o WebFetch):
     ```
     BASE/trade?asset=QQQ&side=buy&qty=N&price=FILL&order_id=limit_order_id
       &strategy=fvg_v1
       &notes=FVG+SL%3DX.XX+TP1%3DY.YY+TP2%3DZ.ZZ+midpoint%3DM.MM+oco_a%3D...+oco_b%3D...+bucket%3D...+risk%3D...+slope%3D...
     ```
  7. Guardar `oco_a_id`, `oco_b_id` en `session_state.position` (incluyendo `fvg_tracking`).
  8. Marcar fvg.status="filled" y remover de `active_fvgs`.
...
**FVG tracking (v2.9.1, 2026-06-09):** Si este cycle tuvo un FVG fill (STEP 8b ejecutado), incluir en indicators JSONB el sub-objeto computado en STEP 8b punto 5:
```json
{
  ...QQQ indicators,
  "tokens_in": N, "tokens_out": N,
  "fvg_tracking": {
    "formed_at_bucket": "10-12" | "12-14" | "14-15:30",
    "vwap_slope": "up" | "flat" | "down" | "unknown",
    "time_since_session_high_min": <int>,
    "risk_size_bucket": "tight" | "mid" | "wide",
    "planned_risk": X.XX
  }
}
```
Si NO hubo FVG fill este cycle, omitir el sub-objeto.
...
JSON_BLOB structure (the `state` JSONB column — do NOT include `date` inside):
{
  ...
  "QQQ":  {
    ...
    "session_high_ts_ET": "HH:MM",         /* v2.9.1 — timestamp del bar que set el session_high */
    "vwap_snapshot_30min": {                /* v2.9.1 — para computar vwap_slope en FVG tracking */
      "value": X, "ts_ET": "HH:MM"
    },
...
## STEP 9c — ADAPTIVE WAKEUP (v2.9.2, 2026-06-10 — FVG OCO-gap fix, Opción B)

Programa el próximo cycle según proximidad a un fill de FVG pendiente. Cierra el gap que el
2026-06-09 dejó 3 fills naked-limit desprotegidos ~14 min (el loop durmió 90s+ mientras el
precio cruzaba las SL).

**RESTRICCIÓN DURA — `ScheduleWakeup` tiene piso 60s** (el runtime clampa a [60,3600]). Los
15-20s de la Opción B original NO son posibles; el gap residual fill→OCO queda acotado a ≤60s,
no 15s. Los guardrails estructurales (max 1 limit activo, sizing 0.025, safety net de posición
desprotegida en STEP 6) acotan la pérdida worst-case en esa ventana.

Calcular delay:
```
pending = [f in active_fvgs if f.status == "pending"]
if pending AND any(abs(live_price − f.midpoint) ≤ 0.50 for f in pending):
    delay = 60     # alert zone — el limit puede fillear ya; armar OCO ASAP el próximo cycle
elif pending:
    delay = 90     # limit vivo pero precio lejos del midpoint
elif position open:
    delay = 90     # OCO broker-side, pero vigilar TP1→BE re-arm
elif abs(live_price − VWAP) ≤ 0.0030 × live_price:
    delay = 90     # zona de alerta VWAP
else:
    delay = 270    # idle
```
`ScheduleWakeup(delay)`. Esto OVERRIDE el 270s default de STEP 0b siempre que exista un FVG
pendiente o una posición abierta.
