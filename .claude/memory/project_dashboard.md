...
| Ruta | Función | Auth |
|---|---|---|
| `GET /api/account` | Proxy Alpaca account + positions (usado por `AccountSummary`) | — |
...
Los endpoints de escritura del agente (`heartbeat`/`log`/`trade`/`trade-exit`/`read`/`memory`) se ELIMINARON el 2026-06-11: el agente local escribe por SQL directo vía Supabase MCP (AGENT_SECRET solo existe en Vercel). Las rutas `/api/db/*` restantes usan `createSupabaseAdmin()` (service_role) — RLS activo con `anon=SELECT only`. Schema completo y grants en `supabase/schema.sql`.
